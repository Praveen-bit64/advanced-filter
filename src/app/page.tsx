...
export default function Page() {
  ...
  const handleSearchType = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const value = e.target.value as "Rent" | "Buy";
    setFilterSearch((prev) => ({
      ...prev,
      searchType: value
    }));
  ...
  return (
    <div className="w-full h-screen flex flex-col justify-center items-center gap-5">
      ...
      <div className="w-full max-w-4xl mx-auto flex justify-between items-center gap-4 bg-white border border-blue-500 p-2 rounded-2xl shadow-md">
        <div className="relative w-[70%] rounded-lg px-3 py-2 flex flex-wrap gap-2 bg-white">
          ...
          {filterSearch.location.map((place, idx) => (
            <div
              key={idx}
              className="bg-blue-100 text-blue-800 text-sm px-3 py-1 rounded-full shadow-sm cursor-pointer"
          ...
          {suggLocation && (
            <div className="absolute w-[70%] max-h-[180px] overflow-y-auto bg-white shadow-md rounded-md mt-10 left-0 z-20 border border-gray-300">
        ...
        <select
          ...
          value={filterSearch.propertyType}
        ...
        >
          ...
          {flatProperty.length > 0 ? (
            flatProperty.map((item, idx) => (
              <option key={idx} value={item}>{item}</option>
            ))
          ) : (
            <option disabled>No suggestions available</option>
          )}
        </select>
      ...
      <div className="">
        <button
          className="p-3 bg-emerald-600 text-white cursor-pointer"
          onClick={() => router.push('/propertyListings')}
        >
          Properties
        </button>
      </div>
